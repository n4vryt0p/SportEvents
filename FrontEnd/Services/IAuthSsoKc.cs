...
public class AuthSsoKc : IAuthSsoKc
{
    ...
    public async Task<ClaimModel?> AddClaims(string userName, string pwd)
    {
        try
        {
            ...
            var modelResult = await _sportEventApi.Ready().LoginUserAsync(new Body2 { Email = userName, Password = pwd });
            ...
            if (modelResult != null)
            {
                ...
                claims.AddRange(new List<Claim>
                {
                    ...
                    new(ClaimTypes.NameIdentifier, modelResult.Id.ToString()),
                    new(ClaimTypes.Name, modelResult.Email),
                    new(ClaimTypes.Email, modelResult.Email)
                });

                claims.Add(new Claim(ClaimTypes.UserData, modelResult.Token));
            }
            else
            {
                return null;
...
public class ClaimModel
{
    ...
    public required ClaimsIdentity claimIdent { get; init; }
}